...
function OpenMenuGarage()
	...
	local elements = {
		{label = "Lista pojazdów", value = 'list_vehicles'},
		{label = "Wprowadź pojazdy", value = 'stock_vehicle'},
		{label = "Odzyskaj pojazd ("..Config.Price.."$)", value = 'return_vehicle'},
	}
...
function ListVehiclesMenu()
	...
	ESX.TriggerServerCallback('eden_garage:getVehicles', function(vehicles)
		...
		for _,v in pairs(vehicles) do
    		...
    		if(v.state)then
    		labelvehicle = vehicleName..': W garazu'
    		
    		else
    		labelvehicle = vehicleName..': Poza garazem'
    		end	
		...
		function(data, menu)
			if(data.current.value.state)then
				...
				TriggerEvent('esx:showNotification', 'Twój pojazd jest wyprowadzony')
			end
		end,
		function(data, menu)
			menu.close()
...
function StockVehicleMenu()
	...
	if IsAnyVehicleNearPoint(this_Garage.DeletePoint.Pos.x,  this_Garage.DeletePoint.Pos.y,  this_Garage.DeletePoint.Pos.z,  3.5) then
		...
		TriggerEvent('esx:showNotification', 'Nie ma pojazdu do wyprowadzenia')
	end
...
AddEventHandler('eden_garage:hasEnteredMarker', function(zone)
	if zone == 'garage' then
		...
		CurrentActionMsg  = "Naciśnij  ~INPUT_PICKUP~ aby otworzyć garaż"
		CurrentActionData = {}
	end
end)
...
function ReturnVehicleMenu()
	...
	ESX.TriggerServerCallback('eden_garage:getOutVehicles', function(vehicles)
		...
		for _,v in pairs(vehicles) do
    		...
    		labelvehicle = vehicleName..': Na zewnątrz'
