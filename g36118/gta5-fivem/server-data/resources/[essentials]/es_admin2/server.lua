...
AddEventHandler('playerConnecting', function(user, set)
	for k,v in ipairs(GetPlayerIdentifiers(source))do
		if isBanned(v) then
			set(GetConvar("es_admin_banreason", "Zostales zbanowany! (Pa Pa)"))
			CancelEvent()
			break
		end
	end
	...
	TriggerEvent('es:getPlayerFromId', source, function(user)
		TriggerEvent('es:canGroupTarget', user.getGroup(), "admin", function(available)
			if available or user.getGroup() == "superadmin" then
				...
				TriggerClientEvent('chatMessage', Source, "SYSTEM", {224, 132, 23}, "You do not have permission to do this")
			end
		end)
	end)
end)
				...
				TriggerEvent('es:canGroupTarget', user.getGroup(), target.getGroup(), function(canTarget)
					if canTarget and available then
						...
						if type == "ban" then
							...
							DropPlayer(id, GetConvar("es_admin_banreason", "Zostales zbanowany! (Pa Pa)"))
						end
					else
						if not available then
							TriggerClientEvent('chatMessage', Source, 'SYSTEM', {224, 132, 23}, "Twoja ranga nie moze uzywac tej komendy.")
						else
							TriggerClientEvent('chatMessage', Source, 'SYSTEM', {224, 132, 23}, "Brak uprawnien.")
						end
					end
				end)
			end)
		...
		TriggerEvent('es:canGroupTarget', user.getGroup(), "mod", function(available)
			...
			if t == "group" then
				if(GetPlayerName(USER) == nil)then
					TriggerClientEvent('chatMessage', source, 'SYSTEM', {224, 132, 23}, "Player not found")
				else
					TriggerEvent("es:getAllGroups", function(groups)
						if(groups[GROUP])then
							TriggerEvent("es:setPlayerData", USER, "group", GROUP, function(response, success)
								...
								TriggerClientEvent('chatMessage', -1, "CONSOLE", {224, 132, 23}, "Group of ^2^*" .. GetPlayerName(tonumber(USER)) .. "^r^0 has been set to ^2^*" .. GROUP)
							end)
						else
							TriggerClientEvent('chatMessage', source, 'SYSTEM', {224, 132, 23}, "Group not found")
						end
					end)
				end
			elseif t == "level" then
				if(GetPlayerName(USER) == nil)then
					...
					if(GROUP ~= nil and GROUP > -1)then
						TriggerEvent("es:setPlayerData", USER, "permission_level", GROUP, function(response, success)
							if(true)then
								TriggerClientEvent('chatMessage', -1, "CONSOLE", {224, 132, 23}, "Permission level of ^2" .. GetPlayerName(tonumber(USER)) .. "^0 has been set to ^2 " .. tostring(GROUP))
							end
						end)	
					else
						TriggerClientEvent('chatMessage', source, 'SYSTEM', {224, 132, 23}, "Wprowadzono niepoprawna liczbe")
					end
				end
			elseif t == "money" then
				if(GetPlayerName(USER) == nil)then
					TriggerClientEvent('chatMessage', source, 'SYSTEM', {224, 132, 23}, "Gracz nie znaleziony.")
				else
					GROUP = tonumber(GROUP)
					if(GROUP ~= nil and GROUP > -1)then
			...
			elseif t == "bank" then
				...
				TriggerClientEvent('chatMessage', source, 'SYSTEM', {224, 132, 23}, "Potrzebujesz SuperAdmina do tego.")
			end
		end)
	end)	
end)
	...
	if commandName == 'setadmin' then
		...
		TriggerEvent("es:setPlayerData", tonumber(args[1]), "permission_level", tonumber(args[2]), function(response, success)
			...
			if(true)then
				...
				TriggerClientEvent('chatMessage', -1, "CONSOLE", {224, 132, 23}, "Level permisji dla gracza ^3" .. GetPlayerName(tonumber(args[1])) .. "^0 zostal ustalony na ^3 " .. args[2])
			end
		end)
			...
			if(groups[args[2]])then
				TriggerEvent("es:setPlayerData", tonumber(args[1]), "group", args[2], function(response, success)
					...
					if(true)then
						...
						TriggerClientEvent('chatMessage', -1, "CONSOLE", {0, 0, 0}, "Ranga ^3" .. GetPlayerName(tonumber(args[1])) .. "^r^0 zostala ustawiona na ^3" .. args[2])
					end
				end)
			else
...
TriggerEvent('es:addCommand', 'admin', function(source, args, user)
	TriggerClientEvent('chatMessage', source, "SYSTEM", {224, 132, 23}, "Level: ^*^2 " .. tostring(user.get('permission_level')))
	TriggerClientEvent('chatMessage', source, "SYSTEM", {224, 132, 23}, "Group: ^*^2 " .. user.getGroup())
end, {help = "Shows what admin level you are and what group you're in"})

-- Default commands
TriggerEvent('es:addCommand', 'report', function(source, args, user)
	table.remove(args, 1)
	TriggerClientEvent('chatMessage', source, "REPORT", {224, 132, 23}, " (^2" .. GetPlayerName(source) .." | "..source.."^0) " .. table.concat(args, " "))

	TriggerEvent("es:getPlayers", function(pl)
		for k,v in pairs(pl) do
			TriggerEvent("es:getPlayerFromId", k, function(user)
				if(user.getPermissions() > 0 and k ~= source)then
					TriggerClientEvent('chatMessage', k, "REPORT", {224, 132, 23}, " (^2" .. GetPlayerName(source) .." | "..source.."^0) " .. table.concat(args, " "))
				end
			end)
		end
	end)
...
TriggerEvent('es:addGroupCommand', 'noclip', "admin", function(source, args, user)
	...
	TriggerClientEvent('chatMessage', source, "SYSTEM", {224, 132, 23}, "Nie masz Permisji!")
	...
	if args[2] then
		if(GetPlayerName(tonumber(args[2])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				table.remove(reason, 1)
				if(#reason == 0)then
					reason = "Kicked: Zostales wyrzucony z Serwera."
				else
					reason = "Kicked: " .. table.concat(reason, " ")
				end

				TriggerClientEvent('chatMessage', -1, "SYSTEM", {224, 132, 23}, "Player ^3" .. GetPlayerName(player) .. "^0 zostal wyrzucony(^3" .. reason .. "^0)")
				DropPlayer(player, reason)
			end)
		else
			TriggerClientEvent('chatMessage', source, "SYSTEM", {224, 132, 23}, "Bledne ID!")
		end
	else
		TriggerClientEvent('chatMessage', source, "SYSTEM", {224, 132, 23}, "^3Bledne ID!")
	end
end, function(source, args, user)
...
TriggerEvent('es:addGroupCommand', 'announce', "admin", function(source, args, user)
	table.remove(args, 1)
	TriggerClientEvent('chatMessage', -1, "ADMINISTRACJA", {224, 132, 23}, "" .. table.concat(args, " "))
end, function(source, args, user)
	TriggerClientEvent('chatMessage', source, "SYSTEM", {224, 132, 23}, "Nie masz Permisji!")
...
TriggerEvent('es:addGroupCommand', 'freeze', "mod", function(source, args, user)
	if args[2] then
		if(GetPlayerName(tonumber(args[2])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('chatMessage', player, "SYSTEM", {224, 132, 23}, "You have been " .. state .. " by ^2" .. GetPlayerName(source))
				TriggerClientEvent('chatMessage', source, "SYSTEM", {224, 132, 23}, "Player ^2" .. GetPlayerName(player) .. "^0 has been " .. state)
			end)
		else
			TriggerClientEvent('chatMessage', source, "SYSTEM", {224, 132, 23}, "Bledne ID!")
		end
	...
	TriggerClientEvent('chatMessage', source, "SYSTEM", {224, 132, 23}, "Insufficienct permissions!")
...
TriggerEvent('es:addGroupCommand', 'bring', "mod", function(source, args, user)
	if args[2] then
		if(GetPlayerName(tonumber(args[2])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('chatMessage', player, "SYSTEM", {224, 132, 23}, "Zostales Przeteleportowany przez ^3" .. GetPlayerName(source))
				TriggerClientEvent('chatMessage', source, "SYSTEM", {224, 132, 23}, "Gracz ^3" .. GetPlayerName(player) .. "^7 Zostal Teleportowany")
			end)
		else
			TriggerClientEvent('chatMessage', source, "SYSTEM", {224, 132, 23}, "Bledne ID!")
		end
	...
	TriggerClientEvent('chatMessage', source, "SYSTEM", {224, 132, 23}, "Nie masz permisji!")
end, {help = "Teleport a user to you", params = {{name = "userid", help = "ID Gracza"}}})
...
TriggerEvent('es:addGroupCommand', 'slap', "admin", function(source, args, user)
	if args[2] then
		if(GetPlayerName(tonumber(args[2])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('chatMessage', player, "SYSTEM", {224, 132, 23}, "You have slapped by ^2" .. GetPlayerName(source))
				TriggerClientEvent('chatMessage', source, "SYSTEM", {224, 132, 23}, "Player ^2" .. GetPlayerName(player) .. "^0 has been slapped")
			end)
		else
			TriggerClientEvent('chatMessage', source, "SYSTEM", {224, 132, 23}, "Bledne ID!")
		end
	...
	TriggerClientEvent('chatMessage', source, "SYSTEM", {224, 132, 23}, "Nie masz permisji!")
end, {help = "Slap a user", params = {{name = "userid", help = "ID Gracza"}}})
...
TriggerEvent('es:addGroupCommand', 'goto', "mod", function(source, args, user)
		...
		if(GetPlayerName(tonumber(args[2])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				if(target)then
					...
					TriggerClientEvent('chatMessage', player, "SYSTEM", {224, 132, 23}, "Zostales teleportowany do ^3" .. GetPlayerName(source))
					TriggerClientEvent('chatMessage', source, "SYSTEM", {224, 132, 23}, "Teleportowales sie do ^3" .. GetPlayerName(player) .. "")
				end
			end)
		else
			TriggerClientEvent('chatMessage', source, "SYSTEM", {224, 132, 23}, "Bledne ID!")
	...
	TriggerClientEvent('chatMessage', source, "SYSTEM", {224, 132, 23}, "Nie masz permisji!")
end, {help = "Teleport to a user", params = {{name = "userid", help = "ID Gracza"}}})

-- Kill yourself
TriggerEvent('es:addCommand', 'die', function(source, args, user)
	TriggerClientEvent('es_admin:kill', source)
	TriggerClientEvent('chatMessage', source, "", {224, 132, 23}, "^1Zabiles sie XD.")
end, {help = "Suicide"})
...
TriggerEvent('es:addGroupCommand', 'slay', "admin", function(source, args, user)
	if args[2] then
		if(GetPlayerName(tonumber(args[2])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('chatMessage', player, "SYSTEM", {224, 132, 23}, "Zostales zabity przez ^3" .. GetPlayerName(source))
				TriggerClientEvent('chatMessage', source, "SYSTEM", {224, 132, 23}, "Gracz ^3" .. GetPlayerName(player) .. "^0 zostal zabity!")
			end)
		else
			TriggerClientEvent('chatMessage', source, "SYSTEM", {224, 132, 23}, "Bledne")
		end
	else
		TriggerClientEvent('chatMessage', source, "SYSTEM", {224, 132, 23}, "Bledne ID!")
	end
	...
	TriggerClientEvent('chatMessage', source, "SYSTEM", {224, 132, 23}, "Nie masz permisji!")
end, {help = "Slay a user", params = {{name = "userid", help = "ID Gracza"}}})
...
TriggerEvent('es:addGroupCommand', 'crash', "superadmin", function(source, args, user)
	if args[2] then
		if(GetPlayerName(tonumber(args[2])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('chatMessage', source, "SYSTEM", {224, 132, 23}, "Gracz ^3" .. GetPlayerName(player) .. "^0 Dostal Crasha od Ciebie (XD).")
			end)
		else
			TriggerClientEvent('chatMessage', source, "SYSTEM", {224, 132, 23}, "Bledne ID!")
		end
	...
	TriggerClientEvent('chatMessage', source, "SYSTEM", {224, 132, 23}, "Brak permisji!")
